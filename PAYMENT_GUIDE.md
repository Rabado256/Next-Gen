...
### b) Frontend fallback in `js/config.js`
...
Keep `js/config.js` and the `.env` public key in sync. The secret key is **never** put
in frontend files.
...
3. Update `.env`, `js/config.js`, and the Vercel Environment Variables with the live keys.
...
| `js/config.js` | Frontend public-key fallback |
...
|---------|--------------------|
| Payment step says "Paystack is not configured yet" | Public key missing/placeholder. Set `PAYSTACK_PUBLIC_KEY` in `.env`, `js/config.js`, and Vercel. |
