...
`js/supabase-config.js` stays gitignored and never reaches the server.
...
|---|---|---|
| `SUPABASE_URL` | yes | Supabase project URL (returned by `/api/config` for the client) |
| `SUPABASE_ANON_KEY` | yes | Supabase anonymous/public key (safe to expose, locked by RLS) |
| `SUPABASE_SERVICE_ROLE_KEY` | yes | Supabase service-role key — server-only. **Never** put in client env |
...
> `js/env.js` fetches `/api/config` on every page and merges the two
> `supabase_*` values into `window.__SUPABASE_CONFIG__`, falling back to the
> local `js/supabase-config.js` only for static-only local dev.
...
Deploy the database schema once (or after schema changes):

```bash
# Run supabase-schema.sql in the Supabase SQL Editor, or use the Supabase CLI:
supabase db push
```

## 3. Post-deploy verification

- [ ] `curl https://<your-domain>/api/config` returns `paystack_public_key`, `supabase_url`, and `supabase_anon_key`
...
- [ ] Review Supabase Auth settings: password min-length policy, email enumeration protection
- [ ] Keep the Supabase service role key secure; audit access in **Supabase Dashboard → Settings → API**
